import { Phone } from "lucide-react";
import { useSiteContent } from "@/contexts/SiteContentContext";
import { cn } from "@/lib/utils";

const FloatingCallButton = ({ className }: { className?: string }) => {
  const { company: COMPANY, siteTop: SITE_TOP } = useSiteContent();
  const phoneHref = `tel:${(COMPANY.phone || "").replace(/[^+\d]/g, "")}`;

  return (
    <a
      href={phoneHref}
      aria-label={`Call ${COMPANY.name} — ${SITE_TOP.line}`}
      className={cn(
        "fixed bottom-5 left-5 z-[60] flex items-center gap-2 rounded-full",
        "bg-[hsl(var(--secondary))] text-[hsl(var(--secondary-foreground))] pl-4 pr-5 py-3.5",
        "font-display font-semibold text-sm shadow-lg shadow-black/30",
        "hover:bg-[hsl(var(--secondary))]/90 hover:scale-[1.02] active:scale-[0.98]",
        "transition-all md:hidden",
        className,
      )}
    >
      <span className="flex h-9 w-9 items-center justify-center rounded-full bg-white/20">
        <Phone className="h-4 w-4" />
      </span>
      Call Now
    </a>
  );
};

export default FloatingCallButton;
