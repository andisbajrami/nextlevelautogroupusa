...
const wrapTheme = (cfg: ThemeConfig) => hydrateMinhsThemeConfig(cfg);
...
export const ThemeProvider: React.FC<ProviderProps> = ({ children, value, onChange, external }) => {
  ...
  useEffect(() => {
    applyThemePalette(document.documentElement, config, "minhs");
  }, [
    config.primaryColor,
  ...
  const resolveServiceImage = (id: string, fallback: string) =>
    config.serviceImages[id] && isAllowedMinhsImage(config.serviceImages[id])
      ? config.serviceImages[id]
      : resolveMinhsServiceImage(id, fallback);
  const resolveServiceSectionImage = (id: string, fallback: string) =>
    config.serviceSectionImages[id] && isAllowedMinhsImage(config.serviceSectionImages[id])
      ? config.serviceSectionImages[id]
      : resolveMinhsServiceSectionImage(id, fallback);
